"""
Rice price seasonality visualization.
Shows price patterns across the year.
"""
...
ax1.scatter(
    ...
    monthly_avg[most_expensive_month],
    s=300,
    color="red",
    marker="*",
    zorder=4,
...
ax1.set_ylabel("Price (UY$)", fontsize=12, fontweight="bold")
ax1.set_title(
    "Rice Seasonality: Average by Month (2016-2025)",
    fontsize=13,
    fontweight="bold",
    pad=15,
)
...
for month in range(1, 13):
    ax1.text(
        month,
        monthly_avg[month] + 0.3,
        f"${monthly_avg[month]:.2f}",
        ha="center",
        fontsize=8,
        fontweight="bold",
    )
...
pivot_data = rice_prices.pivot_table(
    values="price_avg", index="year", columns="month", aggfunc="mean"
)
sns.heatmap(
    pivot_data,
    annot=True,
    ...
    cmap="RdYlGn_r",
    ax=ax2,
    cbar_kws={"label": "Price (UY$)"},
    linewidths=0.5,
)
ax2.set_xlabel("Month", fontsize=12, fontweight="bold")
ax2.set_ylabel("Year", fontsize=12, fontweight="bold")
ax2.set_title(
    "Heatmap: Rice Prices by Month and Year",
    fontsize=13,
    fontweight="bold",
    pad=15,
)
...
box_data = [
    rice_prices[rice_prices["month"] == m]["price_avg"].values for m in range(1, 13)
]
...
ax3.set_ylabel("Price (UY$)", fontsize=11, fontweight="bold")
ax3.set_title("Price Distribution by Month", fontsize=12, fontweight="bold")
...
quarterly = (
    rice_prices.groupby("quarter")["price_avg"].agg(["mean", "std"]).reset_index()
)
...
bars = ax4.bar(
    ...
    quarterly["mean"],
    yerr=quarterly["std"],
    color=colors_quarter,
    alpha=0.7,
    edgecolor="black",
    capsize=5,
)

ax4.set_ylabel("Average Price (UY$)", fontsize=11, fontweight="bold")
ax4.set_title("Quarterly Comparison", fontsize=12, fontweight="bold")
ax4.set_xticks(quarterly["quarter"])
ax4.set_xticklabels(quarters)
ax4.grid(True, alpha=0.3, axis="y")

# Add value labels
for i, (q, mean, std) in enumerate(
    zip(quarterly["quarter"], quarterly["mean"], quarterly["std"])
):
    ax4.text(
        q, mean + std + 0.2, f"${mean:.2f}", ha="center", fontsize=10, fontweight="bold"
    )

plt.suptitle(
    "Seasonality Analysis: White Rice in Uruguay (2016-2025)",
    fontsize=15,
    fontweight="bold",
    y=0.995,
)
