...
# Load data
data_path = Path(__file__).parent.parent / "data" / "processed" / "prices_aggregated_all_years_v4.csv"
...
yearly_agg = rice_prices.groupby('year').agg({
    'price_avg': 'mean',
    'price_median': 'mean',
    'price_min': 'min',
    'price_max': 'max'
...
ax.fill_between(yearly_agg['year'],
...
ax.set_ylabel('Precio ($UY)', fontsize=12, fontweight='bold')
ax.set_title('Evolución del Precio del Arroz en Uruguay (2016-2025)\nTodos los productos de arroz blanco - V4 DATOS LIMPIOS (IQR)',
...
textstr += f'Precio 2016: ${first_avg:.2f}\n'
textstr += f'Precio 2025: ${last_avg:.2f}\n'
textstr += f'Registros: {len(rice_prices):,}\n'
textstr += f'Dataset: V4 (IQR Limpio)'
